...
import { useReleaseYearSongs } from '@/hooks/useReleaseYearSongs';
import { usePlaylists } from '@/hooks/usePlaylists';
import { apiPost } from '@/lib/api';

export type ReleaseYearPreview = {
  id: string;
  name: string;
  total?: number | null;
};

export default function ReleaseYearDetailScreen() {
  const { id, item } = useLocalSearchParams<{
    id: string;
    item: string;
  }>();

  const releaseYearParam = Array.isArray(id) ? id[0] : id;
  const releaseYearItemParam = Array.isArray(item) ? item[0] : item;

  const releaseYear = useMemo<ReleaseYearPreview | undefined>(() => {
    if (!releaseYearItemParam) return undefined;
    try {
      return JSON.parse(releaseYearItemParam) as ReleaseYearPreview;
    } catch {
      return undefined;
    }
  }, [releaseYearItemParam]);
  ...
  const releaseYearValue = useMemo(() => {
    if (!releaseYearParam) return undefined;
    const parsed = Number(releaseYearParam);
    return Number.isNaN(parsed) ? undefined : parsed;
  }, [releaseYearParam]);

  const { data: releaseYearSongsResponse, isLoading: isYearLoading } = useReleaseYearSongs(releaseYearValue);
  ...
  const songs = useMemo(() => releaseYearSongsResponse?.data ?? [], [releaseYearSongsResponse]);
  ...
  useEffect(() => {
    const source = releaseYearSongsResponse?.data;
    if (!source) {
      setTrackPlaylistMap({});
  ...
  }, [releaseYearSongsResponse?.data]);
  const releaseYearLabel =
    releaseYear?.name != null
      ? String(releaseYear.name)
      : releaseYearValue != null
      ? String(releaseYearValue)
      : 'Release Year';

  const totalTracks = releaseYearSongsResponse?.total ?? songs.length;
  const headerMeta = useMemo(() => {
    if (isYearLoading) {
      return 'Loading tracks...';
    }
    return `${totalTracks} ${totalTracks === 1 ? 'track' : 'tracks'}`;
  }, [isYearLoading, totalTracks]);
  ...
  const handleConfirmPlaylists = useCallback(async () => {
    ...
    try {
      ...
      console.error('Failed to add release year song to playlists', error);
    } finally {
      setIsSavingPlaylists(false);
    }
  }, [
      ...
      <Animated.ScrollView
        ...
        <Animated.View style={styles.headerRow} entering={FadeInUp.delay(80).duration(320)}>
          <Surface style={styles.coverCard} elevation={2}>
            <Text style={styles.coverSubtitle}>{releaseYearLabel.slice(0, 2).toUpperCase()}</Text>
          </Surface>
